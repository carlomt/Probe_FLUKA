...
c     rymaxSr=((rwmaxSr/AMELCT)**4-(rwmaxSr/AMELCT)**2)*7.D-2
         ...
         WRITE (LUNOUT,*) "==>                                     <=="
         WRITE (LUNOUT,*) "==> customized source SrY  Gauss        <=="
         WRITE (LUNOUT,*) "==>                                     <=="
         WRITE (LUNOUT,*) "==> by Carlo Mancini, 03/03/2016        <=="
         WRITE (LUNOUT,*) "==>                                     <=="
...
c$$$      CALL FLNRRN(RGAUSS)
c$$$      rRaggioEstr=ABS(RGAUSS)

c$$$      rRaggioEstr=FLRNDM(rdummy) * Twotwo
c$$$      
c$$$      rThetaEstr=FLRNDM(rdummy) * Pipipi
c$$$      rPhiEstr=FLRNDM(rdummy)  * Twopip
c$$$
c$$$      rXestr=rRaggioEstr* SIN(rThetaEstr) * COS(rPhiEstr)
c$$$      rYestr=rRaggioEstr* SIN(rThetaEstr) * SIN(rPhiEstr)
c$$$      rZestr=rRaggioEstr* COS(rThetaEstr)

      rXestr=FLRNDM(rdummy)*Foufou -Twotwo
      rYestr=FLRNDM(rdummy)*Foufou -Twotwo
      rZestr=FLRNDM(rdummy)*Foufou -Twotwo
